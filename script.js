...
(function () {
  ...
  //  - Crypto: Coinbase exchange-rates — one call returns every rate, CORS *.
  //  - EUR:    Frankfurter (ECB reference rate, updates once a day).
  //  - Equities + WTI: prices.json, generated by scripts/fetch-prices.py in the
  //    deploy workflow (browsers can't hit Yahoo directly — no CORS header).
  //    On file:// that fetch fails and the seeds below stay on screen.
  ...
  function fetchFx() {
    fetch("https://api.frankfurter.dev/v1/latest?base=EUR&symbols=USD")
      .then(function (r) { return r.json(); })
      .then(function (d) {
        var usd = d && d.rates && Number(d.rates.USD);
        if (usd > 0) { prices["EUR/USD"] = usd; renderPrices(); }
      })
      .catch(function () {});
  }
  ...
  function boot() {
    ...
    setInterval(fetchFx, 60 * 60000);     // ECB publishes once a day
  }
